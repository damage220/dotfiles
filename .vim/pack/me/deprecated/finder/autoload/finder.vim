...
function! finder#commands(...)
	call call("finder#commands#index", a:000)
endfunction
...
function! finder#fill()
	...
	" direct calling instead of using timer_start causes no blink at first call
	call finder#redraw(v:null)
...
function! finder#redraw(timer)
	...
	if(line('$') > b:queryLine)
		...
		" call setline(b:firstMatchLine, repeat([""], 100))
	endif

	" call cursor(b:firstMatchLine, cursorColumn)
...
function! finder#getMatchesNatively(items)
	let matched = []
	let i = 0

	for item in b:raw
		if(match(item, b:query) != -1)
			call add(matched, a:items[i])
...
function! finder#updateSyntax(pattern)
	...
	if(hlexists("finderSelectedMatch"))
